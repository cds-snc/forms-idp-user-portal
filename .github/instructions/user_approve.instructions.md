...
- **React Compiler**: This project uses React Compiler, which automatically optimizes components. Do NOT use `useCallback` or `useMemo` for optimization - the compiler handles this. However, these hooks are still valid for semantic purposes (e.g., preserving function identity for refs).
