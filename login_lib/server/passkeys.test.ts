...
describe("sendPasskey", () => {
  ...
  beforeEach(async () => {
    ...
    const { getLoginSettings, getUserByID } = await import("../../lib/zitadel");
    const { setSessionAndUpdateCookie } = await import("../../lib/server/cookie");
    const { getSessionCookieById, getSessionCookieByLoginName, getMostRecentSessionCookie } =
